...
public class JavaTasks {
    ...
    static <T extends Comparable<T>> void mergeArrays(T[] first, T[] second) {
        int i = 0;
        int stopLength = 0;
        int finalLength = first.length;
        while (i < second.length) {
            if (stopLength < first.length &&
                    (finalLength == second.length ||
                            first[stopLength].compareTo(second[finalLength]) <= 0)) {
                second[i] = first[stopLength];
                stopLength++;
            } else {
                second[i] = second[finalLength];
                finalLength++;
            }
            i++;
        }
